...
use crate::domain_types::{ColumnName, FinancialSeries, FrequencyMarker, OhlcvSeries, TickSeries};
use polars::prelude::*;
...
impl CsvParser {
    ...
    pub fn parse_tick<F: FrequencyMarker>(
        ...
        // 確保必要的欄位存在
        let required_columns = [time_column, ColumnName::PRICE, ColumnName::VOLUME];
    ...
    fn parse_string_time_column(
        ...
        Err(CsvError::TimestampParseError(format!(
            "無法解析時間欄位 '{}' 的格式",
            time_column
        )))
    }
    ...
    pub fn detect_format(df: &DataFrame) -> CsvResult<CsvFormat> {
        ...
        let has_ohlcv = ohlcv_columns
            .iter()
            .all(|&col| columns.iter().any(|&c| c.to_lowercase() == col));
...
mod tests {
    use super::*;
    use crate::domain_types::{Day, Minute};
    ...
    fn test_parse_missing_required_column() {
        ...
        let result = CsvParser::parse_ohlcv::<Day>(df, "TEST".to_string(), "time", None);
