...
pub use traits::{
    ComposableValidator, ValidationConfig, ValidationRule, Validator
};
pub use report::{ValidationReport, ReportFormatter};
pub use ohlcv_validator::{OhlcvValidator, OhlcvRecord};
pub use tick_validator::{TickValidator, TickRecord};
pub use time_series_validator::{TimeSeriesValidator, TimeSeriesRecord, TimeSeriesStats};
pub use registry::{
    ValidatorRegistry, ValidatorType, ValidatorChain,
    create_ohlcv_registry, create_tick_registry
};
...
pub fn create_default_ohlcv_chain() -> ValidatorChain<OhlcvRecord> {
    ...
    ValidatorChain::new()
        ...
        .add_from_registry(&registry, ValidatorType::TimeSeries, ValidationConfig::default())
...
where
    T: Send + Sync + 'static,
{
    let mut report = ValidationReport::new(validator_name)
