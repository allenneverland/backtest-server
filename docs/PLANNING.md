...
6. **API 友好**：提供 RESTful API 界面，便於與其他系統整合
...
整體架構基於事件驅動和非同步處理，以充分利用現代硬體的多核性能。
...
9. **API服務模組 (api)**：提供外部接口
10. **配置管理模組 (config)**：管理系統配置
11. **伺服器模組 (server)**：管理HTTP服務
12. **回測模組 (backtest)**：協調整個回測流程
13. **存儲系統模組 (storage)**：管理資料儲存

各模組之間通過明確定義的 API 和事件系統進行溝通，降低耦合度。
...
API服務模組提供RESTful接口，允許外部系統與回測伺服器交互。

**主要組件**:
- `handlers.rs`: API 請求處理
- `routes/`: API 路由定義
- `auth.rs`: 認證和授權
- `middleware.rs`: API 中間件
- `error.rs`: API 錯誤處理
