...
impl RabbitMQConnectionManager {
    ...
    pub async fn new(config: RabbitMQConnectionConfig) -> Result<Self, RabbitMQConnectionError> {
        ...
        info!("Successfully created RabbitMQ connection pool with {} connections", config.pool_size);
            ...
            match self.pool.get().await {
                ...
                Err(err) => {
                    ...
                    if retries > max_retries {
                        error!("Failed to get RabbitMQ connection after {} retries: {}", max_retries, err);
