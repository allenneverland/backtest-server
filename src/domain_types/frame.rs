...
fn get_series<'a>(df: &'a DataFrame, column_name: &str) -> PolarsResult<&'a Series> {
    ...
    column.as_series().ok_or_else(|| {
        PolarsError::ColumnNotFound(
            format!(
                "Column '{}' exists but couldn't be converted to Series",
                column_name
            )
            .into(),
        )
...
impl OHLCVFrame {
        ...
        F: Fn(&Series) -> PolarsResult<Series>,
    {
        ...
        let new_columns: Result<Vec<Column>, _> = self
            .df
            .iter()
...
impl TickFrame {
    ...
    pub fn new(mut df: DataFrame, instrument_id: impl Into<String>) -> PolarsResult<Self> {
        ...
        // 檢查必要的列是否存在
        let required_columns = [ColumnName::TIME, ColumnName::PRICE, ColumnName::VOLUME];
...
mod tests {
    ...
    fn test_ohlcv_frame_new() {
        ...
        assert!(ohlcv_frame
            .inner()
            .schema()
            .contains(ColumnName::INSTRUMENT_ID));
    }
