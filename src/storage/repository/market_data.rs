...
use crate::storage::{
    models::market_data::{Tick,MinuteBar, DailyBar, FundamentalIndicator, MinuteBarInsert, InstrumentDailyIndicator, TechnicalIndicator, TickInsert},
...
pub trait MarketDataRepository: Send + Sync + DbExecutor {
    // 分鐘K線數據
    async fn get_minute_bars(&self, instrument_id: i32, time_range: TimeRange, limit: Option<i64>) -> Result<Vec<MinuteBar>>;
    ...
    async fn batch_insert_minute_bars(&self, bars: &[MinuteBarInsert], batch_size: usize) -> Result<()>;
    
    // Tick數據
    async fn get_ticks(&self, instrument_id: i32, time_range: TimeRange, limit: Option<i64>) -> Result<Vec<Tick>>;
    ...
    // 日K線數據（連續聚合）
    async fn get_daily_bars(&self, instrument_id: i32, time_range: TimeRange) -> Result<Vec<DailyBar>>;
    
    // 技術指標
    async fn get_technical_indicators(&self) -> Result<Vec<TechnicalIndicator>>;
    async fn get_technical_indicator_by_id(&self, indicator_id: i32) -> Result<Option<TechnicalIndicator>>;
    async fn get_instrument_daily_indicators(&self, instrument_id: i32, indicator_id: i32, time_range: TimeRange) -> Result<Vec<InstrumentDailyIndicator>>;
    async fn insert_instrument_daily_indicator(&self, indicator: &InstrumentDailyIndicator) -> Result<()>;
    
    // 基本面指標
    async fn get_fundamental_indicators(&self, instrument_id: i32, indicator_type: &str, time_range: TimeRange) -> Result<Vec<FundamentalIndicator>>;
...
impl PgMarketDataRepository {
    ...
    /// 獲取股票分鐘K線數據的時間範圍
    pub async fn get_ohlcv_time_range_for_stock(&self, instrument_id: i32) -> Result<Option<TimeRange>> {
    ...
    /// 獲取股票Tick數據的時間範圍
    pub async fn get_tick_time_range_for_stock(&self, instrument_id: i32) -> Result<Option<TimeRange>> {
        let result = sqlx::query!(
            r#"
            SELECT MIN(time) as min_time, MAX(time) as max_time
            FROM tick
...
impl MarketDataRepository for PgMarketDataRepository {
    ...
    async fn get_minute_bars_head(
        ...
        self.get_minute_bars_head(instrument_id, time_range, limit).await
    }
    ...
    async fn get_minute_bars_tail(
        ...
        self.get_minute_bars_tail(instrument_id, time_range, limit).await
    }
} 