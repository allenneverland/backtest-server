use backtest_server::domain_types::{
    indicators::IndicatorsExt, types::ColumnName, Frequency, Hour, Minute, OhlcvSeries, TickData,
};
use polars::prelude::*;
...
fn test_ohlcv_frame_creation_and_basic_operations() {
    ...
    // Test sorting
    let sorted = OhlcvSeries::<Minute>::new(df, instrument_id.to_string())
        .unwrap()
...
fn test_tick_frame_creation_and_basic_operations() {
    ...
    // Test sorting
    let sorted = TickData::new(df, instrument_id.to_string())
        .unwrap()
        .sort_by_time(true); // Descending
    let sorted_df = sorted.collect().unwrap();
    let sorted_time_col = sorted_df.column(ColumnName::TIME).unwrap();
                ...
                if let Ok(with_macd) = with_rsi.macd(ColumnName::CLOSE, 3, 6, 2, None) {
                    ...
                    if let Ok(with_atr) = with_macd.atr(3, None) {
                        ...
                        if let Ok(with_obv) = with_atr.obv(None) {
                            ...
                            if let Ok(result_frame) =
                                OhlcvSeries::<Minute>::new(with_obv, "AAPL".to_string())
                            {
