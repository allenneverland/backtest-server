...
pub fn datetime_range_to_timestamp_range(start: &DateTime<Utc>, end: &DateTime<Utc>) -> (i64, i64) {
    (datetime_to_timestamp_ms(start), datetime_to_timestamp_ms(end))
}

/// 將時間範圍從 i64 毫秒時間戳轉換為 DateTime<Utc>
/// 用於從計算核心返回到領域模型
pub fn timestamp_range_to_datetime_range(start_ts: i64, end_ts: i64) -> (DateTime<Utc>, DateTime<Utc>) {
    (timestamp_ms_to_datetime(start_ts), timestamp_ms_to_datetime(end_ts))
}
...
mod tests {
    ...
    fn test_datetime_range_conversion() {
        ...
        assert!(start_diff < 2, "開始時間差異應小於2毫秒，實際差異: {}", start_diff);
        assert!(end_diff < 2, "結束時間差異應小於2毫秒，實際差異: {}", end_diff);
    }
}