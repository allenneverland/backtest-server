...
impl RabbitMQPublisher {
    ...
    pub async fn initialize(&self) -> Result<(), RabbitMQError> {
        ...
        if !self.config.exchange_name.is_empty() {
            debug!("Initializing publisher for exchange: {}", self.config.exchange_name);
            ...
            info!("Publisher initialized for exchange: {}", self.config.exchange_name);
        } else {
    ...
    /// 發布消息
    pub async fn publish<T: Serialize>(&self, routing_key: &str, message: &Message<T>) -> Result<(), RabbitMQError> {
        // 確保已初始化
    ...
    /// 發布原始消息
    pub async fn publish_raw(&self, routing_key: &str, payload: &[u8], properties: BasicProperties) -> Result<(), RabbitMQError> {
        // 確保已初始化
        let initialized = *self.initialized.lock().await;
        ...
        debug!("Publishing raw message to exchange: {}, routing_key: {}", exchange_name, routing_key);
