...
pub trait IndicatorsExt {
    ...
    fn stochastic(&self, k_period: usize, d_period: usize, smooth_k: Option<usize>, 
                  alias_prefix: Option<&str>) -> PolarsResult<DataFrame>;
...
impl IndicatorsExt for DataFrame {
    ...
    fn bollinger_bands(&self, column: &str, window: usize, std_dev: f64, alias_prefix: Option<&str>) 
        ...
        // 計算上軌和下軌
        let upper_expr = (col(&middle_alias) + (lit(std_dev) * col("__std")))
            .alias(&upper_alias);
        
        let lower_expr = (col(&middle_alias) - (lit(std_dev) * col("__std")))
            .alias(&lower_alias);
    ...
    fn macd(&self, column: &str, fast_period: usize, slow_period: usize, signal_period: usize, 
        ...
        // 計算 MACD 線
        let macd_expr = (col("__fast_ema") - col("__slow_ema"))
            .alias(&macd_alias);
        ...
        // 計算直方圖(MACD - 信號線)
        let hist_expr = (col(&macd_alias) - col(&signal_alias))
            .alias(&hist_alias);
    ...
    fn stochastic(&self, k_period: usize, d_period: usize, smooth_k: Option<usize>, 
        ...
        let raw_k_expr = ((col(ColumnName::CLOSE) - col("__low_min")) / 
            (col("__high_max") - col("__low_min")) * lit(100.0))
            .alias("__raw_k");
    ...
    fn obv(&self, alias: Option<&str>) -> PolarsResult<DataFrame> {
        ...
        // 3. 計算帶方向的交易量
        let dir_volume_expr = (col("__direction") * col(ColumnName::VOLUME))
            .alias("__dir_volume");
    ...
    fn donchian_channel(&self, window: usize, alias_prefix: Option<&str>) -> PolarsResult<DataFrame> {
        ...
        // 計算中軌((上軌+下軌)/2)
        let middle_expr = ((col(&upper_alias) + col(&lower_alias)) / lit(2.0))
            .alias(&middle_alias);
